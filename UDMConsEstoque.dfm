object DMConsEstoque: TDMConsEstoque
  ...
  object sdsEstoque_Mov: TSQLDataSet
    ...
    CommandText = 
      ...
      'RIO * EM.QTD2) VLR_TOTAL,'#13#10'       em.espessura'#13#10'from ESTOQUE_MOV' +
      ' EM  '#13#10'inner join PRODUTO PRO on (EM.ID_PRODUTO = PRO.ID)'#13#10'left ' +
      'join PESSOA PES on (EM.ID_PESSOA = PES.CODIGO)  '#13#10'left join TAB_' +
      'CFOP CFOP on (EM.ID_CFOP = CFOP.ID)  '#13#10'left join GRUPO GR on (PR' +
      'O.ID_GRUPO = GR.ID)  '#13#10'left join COMBINACAO COMB on (EM.ID_COR =' +
      ' COMB.ID)  '#13#10'left join LOCAL_ESTOQUE LEST on (EM.ID_LOCAL_ESTOQU' +
      'E = LEST.ID)  '#13#10'left join CENTROCUSTO CC on EM.ID_CENTROCUSTO = ' +
      'CC.ID  '#13#10'left join DOCESTOQUE_ITENS DI on DI.ID_MOVESTOQUE = EM.' +
      'ID  '#13#10'left join CENTROCUSTO CC1 on CC1.ID = CC.SUPERIOR  '#13#10'where' +
      ' PRO.ESTOQUE = '#39'S'#39
    MaxBlobSize = -1
    Params = <>
    SQLConnection = dmDatabase.scoDados
